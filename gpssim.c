...
void ConBits(unsigned int  buf[29],unsigned long sbf[5][10]){
    //unsigned long  buf[29];  8字节长度空间buf，每一个字节都是0
    int a=0,b=0;
    ...
    for (int i = 0; i < 2; ++i) {
        if (i==0) {
            for (int ii = 1; ii < 15; ++ii) {
                buf[ii - 1] = buf[ii - 1] | sbf[a][b] >> (32 - (2 + ii * 2));
                buf[ii] = sbf[a][b] << (2 + ii * 2);
                printf("%d:",ii);
                printf("%u\n", buf[ii-1]);
                //printf("%lu\n", buf[ii]);
                if (b == 9) {
            ...
            for (int ii = 0; ii < 15; ++ii) {
                if (ii == 0) {
                    buf[ii + 14] = sbf[a][b] | sbf[a][b + 1] >> (32 - (2 + ii * 2));
                    buf[ii + 15] = sbf[a][b + 1] << (2 + ii * 2);
                    printf("%d:",ii+15);
                    printf("%u\n", buf[ii+14]);
                    //printf("%lu\n", buf[ii+15]);
                    b=b+2;
                    ...
                    if (ii != 14) {
                        buf[ii + 14] = buf[ii + 14] | sbf[a][b] >> (32 - (2 + ii * 2));
                        buf[ii + 15] = sbf[a][b] << (2 + ii * 2);
                        printf("%d:",ii+15);
                        printf("%u\n", buf[ii+14]);
                        //printf("%lu\n", buf[ii+15]);
                        if (b == 9) {
 ...
 */
void eph2sbf(int argc, char*argv[], const ephem_t eph, const ionoutc_t ionoutc, unsigned long sbf[5][N_DWRD_SBF])
{
	...
	sbf[0][1] = 0x1UL<<8; //仅仅只是写了20~22比特位。依次表示第一至第五子帧，第二子帧中时间信息不在这里更新
    ...
    f = atoi(argv[7]);  //标识符f由字符转成int型，输入的第7个参数，选择哪种算法
    klen = atoi(argv[8]);// 第8个参量是截短后输出的秘钥bit长度klen
    n = atoi(argv[9]); //第9个参量是密钥链包含的密钥个数
    rlen = strlen(argv[10]) / 2;//计算输入的第10个参数，初始K0所占字节个数
    Sn = strlen(message);//计算消息的字符串长度

    hexstr_to_bytes(argv[10], rnum, rlen); //将K0转化为字符串,两个字符表示的一个16进制数转化成8比特流然后存入字符串rnum中
    total = klen / 8 * n;//秘钥链字节长度
    keychain = (char*)malloc(total);//分配秘钥链内存空间



    if (f == 0 || f == 1) {
        rv=bdnma_ds_keygen(prvkey, pubkey, f);
        printf("bdnma_ds_keygen: rv = %d\n", rv);
        printf("Private Key:\n");
        printHex(prvkey, 32);
        printf("Public Key:\n");
        printHex(pubkey, 64);
        ...
        rv = bdnma_keychain_gen(prvkey, f, klen, n, rnum, rlen, keychain, sig);
        //rv=bdnma_keychain_gen(prvkey, f, klen, n, keychain, sig);
        printf("bdnma_keychain_gen: rv = %d\n", rv);
        printf("KeyChain:\n");
        for (int i = 0; i < n; ++i) {
            printf("%d:",i+1);
            printHex(keychain + i * klen / 8, klen / 8);
        }
        //printHex(keychain, total);
        printf("Signature:\n");
        printHex(sig, 64);

        key = (char*)malloc(klen/8);//分配每一个k的字节空间大小
        for (int i = 0; i < n ; i++) {
            //
            key=keychain+32*i;
            m = Sn/2;
            buff = (char*)malloc(m);
            hexstr_to_bytes(message, buff, m);
            rv = bdnma_mac(buff, Sn, key, klen, f, mac);
            printf("\nbdnma_mac: rv = %d\n", rv);
            printHex(key, 32);
            printHex(mac, 32);
        }
    ...
    //free(key);
    //free(keychain);
    //现在知道了mac和key是有符号char型数组，存着二进制数，将其放置在子帧4和子帧5中第4~10字，每个字30bits
    //key不能压缩，因此key要完整传输256bits，总容量为7*30*2=420bits，剩余164bits传输mac值，取值高位
    int index = 0;
    int mac_index = 0;
    int sbfindex=3;
    int wordindex=3;
    for (int i = 0; i < 2; ++i) {
        sbf[sbfindex][wordindex]=key[index]<<22|key[index+1]<<14|key[index+2]<<6|key[index+3]>>2;
        index=index+3;
        if (wordindex == 9) {
            sbfindex++;
            wordindex = 3;
        ...
        for (int iii = 0; iii < 3; ++iii) {
            sbf[sbfindex][wordindex] =
                    0x3fffffff &(key[index] << (28 - 2 * iii) | key[index + 1] << (20 - 2 * iii) |
                    key[index + 2] << (12 - 2 * iii) | key[index + 3] <<(4 - 2 * iii)| key[index + 4] >>(4 + 2 * iii));
            index += 4;
            if (wordindex == 9) {
                sbfindex++;
                wordindex = 3;
    ...
    sbf[sbfindex][wordindex]=key[index]<<22|key[index+1]<<14|mac[mac_index]<<6|mac[mac_index+1]>>2;;
    index=index+1;
    mac_index=mac_index+1;
    wordindex++;
    for (int iii = 0; iii < 3; ++iii) {
        sbf[sbfindex][wordindex] =
                (0x3fffffff & mac[mac_index] << (28 - 2 * iii)) | mac[mac_index + 1] << (20 - 2 * iii) |
                mac[mac_index + 2] << (12 - 2 * iii) | mac[mac_index + 3] <<(4 - 2 * iii)| mac[mac_index + 4] >>(4 + 2 * iii);
        mac_index += 4;
        wordindex++;
    }
    sbf[sbfindex][wordindex]=mac[mac_index]<<22|mac[mac_index+1]<<14|mac[mac_index+2]<<6|mac[mac_index+3]>>2;;
    mac_index=mac_index+3;
    wordindex++;
    sbf[sbfindex][wordindex] =
            0x3fffffff & (mac[mac_index] << 28 | mac[mac_index + 1] << 20 | mac[mac_index + 2] << 12 | mac[mac_index + 3] << 4 | mac[mac_index + 4] >> 4);
...
int  allocateChannel(int argc, char*argv[], channel_t *chan, ephem_t *eph, ionoutc_t ionoutc, gpstime_t grx, double *xyz, double elvMask)
{
	int nsat=0;
	int i,sv;
	double azel[2];
			...
			if (allocatedSat[sv]==-1) // Visible but not allocated
			{
				...
				for (i=0; i<MAX_CHAN; i++)
				{
					if (chan[i].prn==0)
					{
						...
						// Generate subframe
						eph2sbf(argc, argv,eph[sv], ionoutc, chan[i].sbf);
...
*/
	...
	// Allocate I/Q buffer
	//void *calloc(size_t n, size_t size)；功 能： 在内存的动态存储区中分配n个长度为size字节的连续空间，函数返回一个指向分配起始地址的指针；如果分配不成功，返回NULL。
	...
	allocateChannel(argc,argv,chan, eph[ieph], ionoutc, grx, xyz[0], elvmask);   //第一次对信道二进制流赋值编码
	...
	for (iumd=1; iumd<numd; iumd++)
	{
		...
		{
			fwrite(iq_buff, 2, 2*iq_buff_size, fp);  //这里使得输出bin文件内容增加
		}

		//
		// Update navigation message and channel allocation every 30 seconds
		//
        //因为0.1秒采样，所以1秒分成10次，判断是否有300次，也就进而将秒乘以10倍，看是不是经过300s，便于操作，0.5其实是为了强制四舍五入进1
		...
		if (igrx%300==0) // Every 30 seconds
		{
				...
				if (eph[ieph+1][sv].vflg==1)
				{
					...
					if (dt<SECONDS_IN_HOUR)
					{
						...
						for (i=0; i<MAX_CHAN; i++)
						{
							...
							if (chan[i].prn!=0)
								eph2sbf(argc, argv,eph[ieph][chan[i].prn-1], ionoutc, chan[i].sbf);
						}
					}
			...
			if (!staticLocationMode)
				allocateChannel(argc,argv,chan, eph[ieph], ionoutc, grx, xyz[iumd], elvmask);
			else
				allocateChannel(argc,argv,chan, eph[ieph], ionoutc, grx, xyz[0], elvmask);
